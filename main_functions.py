...
def cal_performance(pred, made_axiliary_in, gold, trg_pad_idx, args, model, termination_bit_weight=None, smoothing=False):
    ''' Apply label smoothing if needed '''

    loss = cal_loss(pred, made_axiliary_in, gold, trg_pad_idx, args, model, termination_bit_weight, smoothing)
    return loss, None

def cal_loss(pred, made_axiliary_in, gold, trg_pad_idx, args, model, termination_bit_weight=None, smoothing=False):
    ...
    if smoothing:
        ...
        # pred = pred.view(-1, args.max_seq_len, pred.size(-1))

        # eps = 1e-9
        # print('#################### ', ((pred > eps) & (pred < 1-eps)).sum().item(), (pred <= eps).sum().item(), (pred >= 1-eps).sum().item())

        cond_1 = gold != args.pad_idx

        if args.feed_graph_length:
            cond_1[:, :, 0] = False

        pred_1 = torch.tril(pred.masked_fill(~cond_1, 0), diagonal=0)
        gold_1 = torch.tril(gold.masked_fill(~cond_1, 0), diagonal=0)
        ...
        if args.use_MADE:
            ...
            tmp = model.get_MADE_output(torch.cat([made_axiliary_in, prepare_for_MADE(gold_all_zeros, args)], dim=2))
            if args.scale_prj:
        ...
        if args.feed_graph_length:
            ...
            cond_0 = gold[:,:,0] != args.pad_idx
        cond_0[:, 0] = False
        cond_2 = cond_0.unsqueeze(-1).repeat(1, 1, gold.size(-1))
        if args.feed_graph_length:
            cond_2[:,:,0] = False
        pred_2 = torch.tril(pred_all_zeros.masked_fill(~cond_2, 0), diagonal=0)
...
def generate_graph_exact(gg_model, args):
    ...
    # src_seq = args.pad_idx * torch.ones((args.test_batch_size, args.max_seq_len, args.max_num_node + 1),
    ...
    for i in range(args.max_seq_len - 1):
        ...
        src_seq[new_finished_idx, i + 1, 1:] = args.pad_idx
        ...
        for i in range(args.max_seq_len - 1):
            ...
            while remainder_idx.sum().item() > 0:
                ...
                if args.use_MADE:
                    ...
                    #     gold = args.pad_idx * torch.ones(remainder_idx.sum().item(), src_seq.size(2) - 1).to(args.device)
                    #     term_bits = torch.rand([remainder_idx.sum().item()], device=args.device) < pred_probs[
                    #         remainder_idx, i, 0]
                    #     pred_probs = pred_probs[:, :, 1:]
                    # else:
                    gold = args.pad_idx * torch.ones(remainder_idx.sum().item(), src_seq.size(2)).to(args.device)
                    for j in range(i + 1):
                        if args.use_max_prev_node and i > args.max_prev_node and j > 0 and j < i - args.max_prev_node + 1:
                ...
                if args.allow_all_zeros:
                    ...
                    if num_trials >= args.max_num_generate_trials:
                        ...
                        if i+2 < args.max_seq_len:
                            src_seq[remainder_idx, i+2:, 0] = args.pad_idx
...
def just_test(gg_model, dataset, args):
